###Samie Updated with new data June 2026###
# housekeeping
rm(list=ls()) 
...
hrm10 <- lm(above9_days~cancov.10m, data=combined_df)
>>>>>>> Stashed changes
...
###Census Information and community impact index###
...
joined <- st_join(sensors_sf, cbg, join = st_within) 
...
combined_tree_CIscores <- left_join(all_tree, sensor_CI_score, by="ID")
combined_tree_CIscores <- na.omit(combined_tree_CIscores)
combined_tree_CIscores <- combined_tree_CIscores %>%
  mutate(level = case_when(
    score==0 ~ "At County Average",
    score<0 ~ "Below County Average",
    TRUE ~ "Above County Average"
  ))
##The z-scores range from negative to positive values, where:
#negative value = below county avg, 
#0 = at county avg,
#positive val = above county avg)
...
##updated ggplot 2026###
x<- combined_tree_CIscores %>% data.frame() %>% filter(radius_m == "10") %>% ###
ggplot(aes(x=level, y = (total_canopy)))+
  geom_boxplot(na.rm=TRUE)+
